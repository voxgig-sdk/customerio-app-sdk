...
func TestNewsletterDirect(t *testing.T) {
	t.Run("direct-list-newsletter", func(t *testing.T) {
		setup := newsletterDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-newsletter", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-newsletter", func(t *testing.T) {
		setup := newsletterDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-newsletter", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type newsletterDirectSetupResult struct {
...
func newsletterDirectSetup(mockres any) *newsletterDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_NEWSLETTER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_NEWSLETTER_ENTID"]; ok {
		...
		return &newsletterDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &newsletterDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
