...
func TestCollectionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_COLLECTION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		collectionRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "collection"}), "collection_ref01"))
		...
		collectionRef01MarkdefUp0Name := "name"
...
func collectionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_COLLECTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_COLLECTION_ENTID"])
