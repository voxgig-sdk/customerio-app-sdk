...
func TestDesignStudioDirect(t *testing.T) {
	t.Run("direct-list-design_studio", func(t *testing.T) {
		setup := design_studioDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-design_studio", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-design_studio", func(t *testing.T) {
		setup := design_studioDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-design_studio", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type design_studioDirectSetupResult struct {
...
func design_studioDirectSetup(mockres any) *design_studioDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_DESIGN_STUDIO_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_DESIGN_STUDIO_ENTID"]; ok {
		...
		return &design_studioDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &design_studioDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
