...
func TestMessageDirect(t *testing.T) {
	t.Run("direct-list-message", func(t *testing.T) {
		setup := messageDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-message", _mode); _shouldSkip {
			if _reason == "" {
...
func messageDirectSetup(mockres any) *messageDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_MESSAGE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_MESSAGE_ENTID"]; ok {
