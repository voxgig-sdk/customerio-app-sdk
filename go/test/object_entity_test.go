...
func TestObjectEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Object(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Object(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_OBJECT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		objectRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "object"}), "object_ref01"))
		objectRef01Data["object_id"] = setup.idmap["object01"]

		objectRef01DataResult, err := objectRef01Ent.Create(objectRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		objectRef01Data = core.ToMapAny(entityData(objectRef01DataResult))
		...
		objectRef01DataDt0LoadResult := core.ToMapAny(entityData(objectRef01DataDt0Loaded))
...
func objectBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_OBJECT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_OBJECT_ENTID"])
