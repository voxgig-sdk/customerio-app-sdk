...
func TestLinkEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_LINK_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		linkRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.link")))
		var linkRef01Data map[string]any
		if len(linkRef01DataRaw) > 0 {
			linkRef01Data = core.ToMapAny(linkRef01DataRaw[0][1])
		}
...
func linkBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"link01", "link02", "link03", "broadcast01", "broadcast02", "broadcast03", "campaign01", "campaign02", "campaign03", "newsletter01", "newsletter02", "newsletter03", "transactional01", "transactional02", "transactional03", "action01", "action02", "action03", "content01", "content02", "content03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_LINK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_LINK_ENTID"])
