...
func TestTriggerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_TRIGGER_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		triggerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.trigger")))
		var triggerRef01Data map[string]any
		if len(triggerRef01DataRaw) > 0 {
			triggerRef01Data = core.ToMapAny(triggerRef01DataRaw[0][1])
		}
...
func triggerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"trigger01", "trigger02", "trigger03", "campaign01", "campaign02", "campaign03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_TRIGGER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_TRIGGER_ENTID"])
