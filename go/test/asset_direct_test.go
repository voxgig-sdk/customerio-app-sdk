...
func TestAssetDirect(t *testing.T) {
	t.Run("direct-list-asset", func(t *testing.T) {
		setup := assetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-asset", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-asset", func(t *testing.T) {
		setup := assetDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-asset", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type assetDirectSetupResult struct {
...
func assetDirectSetup(mockres any) *assetDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_ASSET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_ASSET_ENTID"]; ok {
		...
		return &assetDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &assetDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
