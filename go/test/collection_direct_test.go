...
func TestCollectionDirect(t *testing.T) {
	...
	t.Run("direct-load-collection", func(t *testing.T) {
		setup := collectionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-collection", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func collectionDirectSetup(mockres any) *collectionDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_COLLECTION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_COLLECTION_ENTID"]; ok {
