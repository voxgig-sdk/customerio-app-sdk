...
func TestMessageEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Message(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Message(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_MESSAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		messageRef01Match := map[string]any{}

		messageRef01ListResult, err := messageRef01Ent.List(messageRef01Match, nil)
		if err != nil {
			t.Fatalf("list failed: %v", err)
		}
		_, messageRef01ListOk := messageRef01ListResult.([]any)
		if !messageRef01ListOk {
			t.Fatalf("expected list result to be an array, got %T", messageRef01ListResult)
		}
...
func messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_MESSAGE_ENTID"])
