...
func TestSegmentDirect(t *testing.T) {
	t.Run("direct-list-segment", func(t *testing.T) {
		setup := segmentDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-segment", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-segment", func(t *testing.T) {
		setup := segmentDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-segment", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		result, err := client.Direct(map[string]any{
			"path":   "v1/segments/{id}/membership",
			"method": "GET",
			"params": params,
			"query":  query,
		})
...
type segmentDirectSetupResult struct {
...
func segmentDirectSetup(mockres any) *segmentDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_SEGMENT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_SEGMENT_ENTID"]; ok {
		...
		return &segmentDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &segmentDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
