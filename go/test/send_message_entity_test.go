...
func TestSendMessageEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_SEND_MESSAGE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if sendMessageRef01Data["id"] == nil {
...
func send_messageBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"send_message01", "send_message02", "send_message03", "campaign01", "campaign02", "campaign03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_SEND_MESSAGE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_SEND_MESSAGE_ENTID"])
