...
func TestEmailEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_EMAIL_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		emailRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.email")))
...
func emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_EMAIL_ENTID"])
