...
func TestWorkspaceEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_WORKSPACE_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		workspaceRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.workspace")))
		var workspaceRef01Data map[string]any
		if len(workspaceRef01DataRaw) > 0 {
			workspaceRef01Data = core.ToMapAny(workspaceRef01DataRaw[0][1])
		}
		// Discard guards against Go's unused-var check when the flow's steps
		// happen not to consume the bootstrap data (e.g. list-only flows).
		_ = workspaceRef01Data
		...
		_, workspaceRef01ListOk := workspaceRef01ListResult.([]any)
...
func workspaceBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_WORKSPACE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_WORKSPACE_ENTID"])
