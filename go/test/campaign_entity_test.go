...
func TestCampaignEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_CAMPAIGN_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		campaignRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.campaign")))
		var campaignRef01Data map[string]any
		if len(campaignRef01DataRaw) > 0 {
			campaignRef01Data = core.ToMapAny(campaignRef01DataRaw[0][1])
		}
...
func campaignBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"campaign01", "campaign02", "campaign03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_CAMPAIGN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_CAMPAIGN_ENTID"])
