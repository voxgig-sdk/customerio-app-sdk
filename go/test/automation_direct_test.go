...
func TestAutomationDirect(t *testing.T) {
	t.Run("direct-list-automation", func(t *testing.T) {
		setup := automationDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-automation", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"campaign01"} {
	...
	t.Run("direct-load-automation", func(t *testing.T) {
		setup := automationDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-automation", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
		...
		if setup.live {
			...
			listParams["campaign_id"] = setup.idmap["campaign01"]
...
type automationDirectSetupResult struct {
...
func automationDirectSetup(mockres any) *automationDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_AUTOMATION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_AUTOMATION_ENTID"]; ok {
		...
		return &automationDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &automationDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
