...
func TestExportDirect(t *testing.T) {
	t.Run("direct-list-export", func(t *testing.T) {
		setup := exportDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-export", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-export", func(t *testing.T) {
		setup := exportDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-export", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type exportDirectSetupResult struct {
...
func exportDirectSetup(mockres any) *exportDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_EXPORT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_EXPORT_ENTID"]; ok {
		...
		return &exportDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &exportDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
