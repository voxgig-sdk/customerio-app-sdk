...
func TestSenderIdentityDirect(t *testing.T) {
	t.Run("direct-list-sender_identity", func(t *testing.T) {
		setup := sender_identityDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-sender_identity", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-sender_identity", func(t *testing.T) {
		setup := sender_identityDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-sender_identity", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type sender_identityDirectSetupResult struct {
...
func sender_identityDirectSetup(mockres any) *sender_identityDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CUSTOMERIO_APP_TEST_SENDER_IDENTITY_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CUSTOMERIO_APP_TEST_SENDER_IDENTITY_ENTID"]; ok {
		...
		return &sender_identityDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &sender_identityDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
