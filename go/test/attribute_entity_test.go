...
func TestAttributeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_ATTRIBUTE_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		attributeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.attribute")))
...
func attributeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"attribute01", "attribute02", "attribute03", "object01", "object02", "object03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_ATTRIBUTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_ATTRIBUTE_ENTID"])
