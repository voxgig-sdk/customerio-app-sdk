...
func TestContentEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_CONTENT_ENTID JSON to run live")
			return
		}
...
func contentBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"content01", "content02", "content03", "newsletter01", "newsletter02", "newsletter03", "transactional01", "transactional02", "transactional03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_CONTENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_CONTENT_ENTID"])
