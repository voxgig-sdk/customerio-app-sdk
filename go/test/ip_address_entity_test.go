...
func TestIpAddressEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_IP_ADDRESS_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		ipAddressRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.ip_address")))
...
func ip_addressBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CUSTOMERIO_APP_TEST_IP_ADDRESS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CUSTOMERIO_APP_TEST_IP_ADDRESS_ENTID"])
