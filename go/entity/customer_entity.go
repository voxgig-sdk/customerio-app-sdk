...
func NewCustomerEntity(client *core.CustomerioAppSDK, entopts map[string]any) *CustomerEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *CustomerEntity) GetName() string { return e.name }
...
func (e *CustomerEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *CustomerEntity) Deleted() bool {
	return e.deleted
}


func (e *CustomerEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewCustomerEntity(e.client, opts)
}

func (e *CustomerEntity) Data(args ...any) any {
...
func (e *CustomerEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Customer; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *CustomerEntity) DataTyped(data ...Customer) Customer {
	if len(data) > 0 {
		return typedFrom[Customer](e.Data(asMap(data[0])))
	}
	return typedFrom[Customer](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Customer (all fields
// optional at the wire level).
func (e *CustomerEntity) MatchTyped(match ...Customer) Customer {
	if len(match) > 0 {
		return typedFrom[Customer](e.Match(asMap(match[0])))
	}
	return typedFrom[Customer](e.Match())
}
...
func (e *CustomerEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *CustomerEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
	ctx := utility.MakeContext(map[string]any{
		"opname":   "list",
		"ctrl":     ctrl,
...
// CustomerListMatch and returns []Customer. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *CustomerEntity) ListTyped(reqmatch CustomerListMatch, ctrl map[string]any) ([]Customer, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Customer](res), nil
}
...
func (e *CustomerEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *CustomerEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
