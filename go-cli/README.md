...
boru-driven command-line client **and** interactive REPL for the CustomerioApp
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/customerio-app-cli)
make build

# 2. See usage (words, entities, env vars)
./customerio-app-cli --help
...
CUSTOMERIO_APP_BASE=https://api.example.com ./customerio-app-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/customerio-app-cli
   ```
...
Run with no arguments to open a REPL (prompt `customerio-app>`). Each line is
...
below — this SDK exposes 53 entities.
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/customerio-app-cli`. |
...
The 53 entities this SDK exposes (any is valid as `<entity>`):
...
(`./customerio-app-cli <expr>`) and interactively (the REPL), and expressions compose the
