// Command CustomerioApp-cli is a CLI + REPL for the CustomerioApp SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CUSTOMERIO_APP_APIKEY (api key), CUSTOMERIO_APP_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CUSTOMERIO_APP_APIKEY carries the API key and
	// CUSTOMERIO_APP_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CUSTOMERIO_APP_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CUSTOMERIO_APP_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCustomerioAppSDK(opts)
