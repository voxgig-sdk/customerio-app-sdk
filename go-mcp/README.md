...
[MCP](https://modelcontextprotocol.io) server exposing the CustomerioApp SDK as
two agent tools — `customerio-app_list` and `customerio-app_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/customerio-app-mcp)
make build

# 2. Provide credentials via the environment
export CUSTOMERIO_APP_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user customerio-app \
  -- /absolute/path/to/customerio-app-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./customerio-app-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// customerio-app_list: first page of records
{ "entity": "activity" }
{ "entity": "activity", "query": { } }

// customerio-app_load: one record by id
{ "entity": "asset", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/customerio-app-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user customerio-app \
     -- "$PWD"/dist/*/customerio-app-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `customerio-app_list` and `customerio-app_load` tools now appear
   in new sessions. Ask the agent to *"list activity using customerio-app"*
   and it calls `customerio-app_list` with `{"entity":"activity"}`.
...
```sh
export CUSTOMERIO_APP_APIKEY=sk_live_xxx            # API key
export CUSTOMERIO_APP_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `customerio-app_list` tool
...
### Call the `customerio-app_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `customerio-app_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `customerio-app_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Field | Type | Notes |
|-------|------|-------|
| `entity` | string | One of the 53 supported entities (see below). |
...
| Variable | Purpose |
|----------|---------|
| `CUSTOMERIO_APP_APIKEY` | API key sent with every request. |
| `CUSTOMERIO_APP_BASE` | Optional override of the API base URL. |

### Entities

The 53 entities valid as the `entity` argument:
...
```sh
./customerio-app-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"customerio-app_load","arguments":{"entity":"asset","query":{"id":1}}}}'
```
