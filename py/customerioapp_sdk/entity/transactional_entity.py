...
from customerioapp_sdk.customerioapp_types import (
    Transactional,
    TransactionalLoadMatch,
    TransactionalListMatch,
    TransactionalUpdateData,
)
...
class TransactionalEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TransactionalEntity(self._client, opts)
    ...
    def data_get(self) -> Transactional:
    ...
    def match_get(self) -> Transactional:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Transactional:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Transactional().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Transactional]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Transactional().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
    ...
    def update(self, reqdata: TransactionalUpdateData, ctrl=None) -> Transactional:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
