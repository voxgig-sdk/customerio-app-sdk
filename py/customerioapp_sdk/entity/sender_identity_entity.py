...
class SenderIdentityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SenderIdentityEntity(self._client, opts)
    ...
    def data_get(self) -> SenderIdentity:
    ...
    def match_get(self) -> SenderIdentity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> SenderIdentity:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.SenderIdentity().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[SenderIdentity]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.SenderIdentity().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
