...
class LiveNotificationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return LiveNotificationEntity(self._client, opts)
    ...
    def data_get(self) -> LiveNotification:
    ...
    def match_get(self) -> LiveNotification:
    ...
    def load(self, reqmatch=None, ctrl=None) -> LiveNotification:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.LiveNotification().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: LiveNotificationCreateData, ctrl=None) -> LiveNotification:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
