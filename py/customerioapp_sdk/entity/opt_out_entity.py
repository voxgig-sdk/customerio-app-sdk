...
class OptOutEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OptOutEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[OptOut]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.OptOut().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: OptOutUpdateData, ctrl=None) -> OptOut:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
