...
class ActivityEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ActivityEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Activity]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Activity().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
