...
class TestWorkspaceDirect:
    ...
    def test_should_direct_list_workspace(self):
        setup = _workspace_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-workspace", "live" if setup["live"] else "unit")
        if _skip:
...
def _workspace_direct_setup(mockres):
