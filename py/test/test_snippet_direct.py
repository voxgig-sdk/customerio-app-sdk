...
class TestSnippetDirect:
    ...
    def test_should_direct_list_snippet(self):
        setup = _snippet_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-snippet", "live" if setup["live"] else "unit")
        if _skip:
...
def _snippet_direct_setup(mockres):
