...
class TestInAppEntity:

    def test_should_create_instance(self):
        testsdk = CustomerioAppSDK.test(None, None)
        ent = testsdk.InApp(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _in_app_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "in_app." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_IN_APP_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        in_app_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.in_app")))
        in_app_ref01_data = None
        if len(in_app_ref01_data_raw) > 0:
            in_app_ref01_data = helpers.to_map(in_app_ref01_data_raw[0][1])



def _in_app_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/in_app/InAppTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CUSTOMERIO_APP_TEST_IN_APP_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_IN_APP_ENTID"))
