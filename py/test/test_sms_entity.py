...
class TestSmsEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Sms(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _sms_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "sms." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_SMS_ENTID JSON to run live")
        client = setup["client"]
        ...
        sms_ref01_data = None
        if len(sms_ref01_data_raw) > 0:
            sms_ref01_data = helpers.to_map(sms_ref01_data_raw[0][1])



def _sms_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/sms/SmsTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_SMS_ENTID"))
