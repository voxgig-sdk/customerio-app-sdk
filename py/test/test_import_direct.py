...
class TestImportDirect:
    ...
    def test_should_direct_load_import(self):
        setup = _import_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-import", "live" if setup["live"] else "unit")
        if _skip:
...
def _import_direct_setup(mockres):
