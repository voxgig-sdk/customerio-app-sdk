...
class TestEndEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.End(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _end_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "end." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_END_ENTID JSON to run live")
        client = setup["client"]
        ...
        end_ref01_data = None
        if len(end_ref01_data_raw) > 0:
            end_ref01_data = helpers.to_map(end_ref01_data_raw[0][1])



def _end_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/end/EndTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_END_ENTID"))
