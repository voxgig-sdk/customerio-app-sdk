...
class TestOptOutDirect:

    def test_should_direct_list_opt_out(self):
        setup = _opt_out_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-opt_out", "live" if setup["live"] else "unit")
        if _skip:
...
def _opt_out_direct_setup(mockres):
