    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_IMPORT_ENTID JSON to run live")
        client = setup["client"]
        ...
        import_ref01_data = helpers.to_map(runner.entity_data(import_ref01_ent.create(import_ref01_data, None)))
        assert import_ref01_data is not None
        assert import_ref01_data["id"] is not None

        # LOAD
        import_ref01_match_dt0 = {
            "id": import_ref01_data["id"],
        }
        import_ref01_data_dt0_loaded = import_ref01_ent.load(import_ref01_match_dt0, None)
        import_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(import_ref01_data_dt0_loaded))
        assert import_ref01_data_dt0_load_result is not None
        assert import_ref01_data_dt0_load_result["id"] == import_ref01_data["id"]
...
def _import_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "CUSTOMERIO_APP_TEST_IMPORT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_IMPORT_ENTID"))
