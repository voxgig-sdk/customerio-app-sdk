    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_LINK_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        link_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.link")))
        link_ref01_data = None
        if len(link_ref01_data_raw) > 0:
...
def _link_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "CUSTOMERIO_APP_TEST_LINK_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_LINK_ENTID"))
