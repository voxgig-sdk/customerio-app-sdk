...
class TestTestGroupEntity:

    def test_should_create_instance(self):
        testsdk = CustomerioAppSDK.test(None, None)
        ent = testsdk.TestGroup(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _test_group_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "test_group." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CUSTOMERIO_APP_TEST_TEST_GROUP_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        test_group_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.test_group")))
        test_group_ref01_data = None
        if len(test_group_ref01_data_raw) > 0:
            test_group_ref01_data = helpers.to_map(test_group_ref01_data_raw[0][1])



def _test_group_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/test_group/TestGroupTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "CUSTOMERIO_APP_TEST_TEST_GROUP_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("CUSTOMERIO_APP_TEST_TEST_GROUP_ENTID"))
