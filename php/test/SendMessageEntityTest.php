...
class SendMessageEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = CustomerioAppSDK::test(null, null);
        $ent = $testsdk->SendMessage(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = send_message_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "send_message." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_SEND_MESSAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $send_message_ref01_ent = $client->SendMessage(null);
        ...
        $this->assertNotNull($send_message_ref01_data["id"]);
...
function send_message_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/send_message/SendMessageTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["send_message01", "send_message02", "send_message03", "campaign01", "campaign02", "campaign03"] as $k) {
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_SEND_MESSAGE_ENTID");
