...
class AssetEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_ASSET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function asset_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["asset01", "asset02", "asset03"] as $k) {
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_ASSET_ENTID");
