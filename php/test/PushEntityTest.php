...
class PushEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_PUSH_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function push_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["push01", "push02", "push03"] as $k) {
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_PUSH_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOMERIO_APP_TEST_PUSH_ENTID"]);
