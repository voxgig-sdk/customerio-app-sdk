...
class CustomerEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Customer(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Customer(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_CUSTOMER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function customer_basic_setup($extra)
{
    Runner::load_env_local();

    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/customer/CustomerTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["customer01", "customer02", "customer03"] as $k) {
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_CUSTOMER_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "CUSTOMERIO_APP_TEST_CUSTOMER_ENTID" => $idmap,
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOMERIO_APP_TEST_CUSTOMER_ENTID"]);
