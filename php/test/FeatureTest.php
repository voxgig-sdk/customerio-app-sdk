...
// CustomerioApp SDK feature test
//
...
class FtCtrl extends CustomerioAppControl
{
...
class FtClient
{
    ...
    private CustomerioAppUtility $utility;

    public function __construct(CustomerioAppUtility $utility, array $options)
    {
    ...
    public function get_utility(): CustomerioAppUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CustomerioAppContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CustomerioAppContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CustomerioAppContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CustomerioAppConfig::make_feature($name);
    ...
    private static function buildUrl(CustomerioAppSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CustomerioAppError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CustomerioAppResponse($fetched);
        }
    ...
    private function fail(CustomerioAppContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CustomerioAppContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
