...
class CollectionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_COLLECTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $collection_ref01_markdef_up0_value = "Mark01-collection_ref01_" . $setup["now"];
        ...
        $collection_ref01_ent->remove($collection_ref01_match_rm0, null);

        // LIST
        $collection_ref01_match_rt0 = [];

        $collection_ref01_list_rt0_result = $collection_ref01_ent->list($collection_ref01_match_rt0, null);
        $this->assertIsArray($collection_ref01_list_rt0_result);
...
function collection_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_COLLECTION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOMERIO_APP_TEST_COLLECTION_ENTID"]);
