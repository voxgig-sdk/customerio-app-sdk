...
class ActionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_ACTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $action_ref01_data = null;
        if (count($action_ref01_data_raw) > 0) {
...
function action_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["action01", "action02", "action03", "broadcast01", "broadcast02", "broadcast03", "campaign01", "campaign02", "campaign03"] as $k) {
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_ACTION_ENTID");
