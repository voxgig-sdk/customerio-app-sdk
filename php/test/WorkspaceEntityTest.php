...
class WorkspaceEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_WORKSPACE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $workspace_ref01_data = null;
        if (count($workspace_ref01_data_raw) > 0) {
...
function workspace_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_WORKSPACE_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOMERIO_APP_TEST_WORKSPACE_ENTID"]);
