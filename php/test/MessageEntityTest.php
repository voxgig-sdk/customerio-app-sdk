...
class MessageEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Message(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Message(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_MESSAGE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $message_ref01_data = null;
        if (count($message_ref01_data_raw) > 0) {
        ...
        $message_ref01_list_result = $message_ref01_ent->list($message_ref01_match, null);
        $this->assertIsArray($message_ref01_list_result);
...
function message_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CUSTOMERIO_APP_TEST_MESSAGE_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "CUSTOMERIO_APP_TEST_MESSAGE_ENTID" => $idmap,
    ...
    $idmap_resolved = Helpers::to_map(
        $env["CUSTOMERIO_APP_TEST_MESSAGE_ENTID"]);
