...
describe("NewsletterMetricDirect", function()
  it("should direct-list-newsletter_metric", function()
    local setup = newsletter_metric_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-newsletter_metric", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"newsletter_metric01"}) do
  ...
  it("should direct-load-newsletter_metric", function()
    local setup = newsletter_metric_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-newsletter_metric", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
