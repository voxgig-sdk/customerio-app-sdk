...
describe("SenderIdentityDirect", function()
  it("should direct-list-sender_identity", function()
    local setup = sender_identity_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-sender_identity", setup.live and "live" or "unit")
  ...
  it("should direct-load-sender_identity", function()
    local setup = sender_identity_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-sender_identity", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
