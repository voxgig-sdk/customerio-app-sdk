...
describe("WorkspaceEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_WORKSPACE_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local workspace_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.workspace")))
    local workspace_ref01_data = nil
    if #workspace_ref01_data_raw > 0 then
      workspace_ref01_data = helpers.to_map(workspace_ref01_data_raw[1][2])
    end
...
function workspace_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CUSTOMERIO_APP_TEST_WORKSPACE_ENTID")
