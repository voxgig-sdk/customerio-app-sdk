...
describe("ActionEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_ACTION_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local action_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.action")))
    local action_ref01_data = nil
    if #action_ref01_data_raw > 0 then
      action_ref01_data = helpers.to_map(action_ref01_data_raw[1][2])
    end
...
function action_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CUSTOMERIO_APP_TEST_ACTION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["CUSTOMERIO_APP_TEST_ACTION_ENTID"])
