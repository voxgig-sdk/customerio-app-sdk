...
describe("ActivityEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Activity(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Activity(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_ACTIVITY_ENTID JSON to run live")
      return
    end
    ...
    local activity_ref01_match = {}

    local activity_ref01_list_result, err = activity_ref01_ent:list(activity_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(activity_ref01_list_result)
...
function activity_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CUSTOMERIO_APP_TEST_ACTIVITY_ENTID")
