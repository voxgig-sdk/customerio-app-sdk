...
describe("ReportingWebhookDirect", function()
  it("should direct-list-reporting_webhook", function()
    local setup = reporting_webhook_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-reporting_webhook", setup.live and "live" or "unit")
  ...
  it("should direct-load-reporting_webhook", function()
    local setup = reporting_webhook_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-reporting_webhook", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
