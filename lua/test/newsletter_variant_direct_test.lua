...
describe("NewsletterVariantDirect", function()
  it("should direct-list-newsletter_variant", function()
    local setup = newsletter_variant_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-newsletter_variant", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"newsletter_variant01"}) do
  ...
  it("should direct-load-newsletter_variant", function()
    local setup = newsletter_variant_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-newsletter_variant", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
