...
describe("NewsletterDirect", function()
  it("should direct-list-newsletter", function()
    local setup = newsletter_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-newsletter", setup.live and "live" or "unit")
  ...
  it("should direct-load-newsletter", function()
    local setup = newsletter_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-newsletter", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
