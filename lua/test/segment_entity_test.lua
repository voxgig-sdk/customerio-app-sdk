...
describe("SegmentEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Segment(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Segment(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set CUSTOMERIO_APP_TEST_SEGMENT_ENTID JSON to run live")
      return
    end
    ...
    local segment_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.segment"), "segment_ref01"))

    local segment_ref01_data_result, err = segment_ref01_ent:create(segment_ref01_data, nil)
    assert.is_nil(err)
    segment_ref01_data = helpers.to_map(type(segment_ref01_data_result) == 'table' and segment_ref01_data_result.data_get and segment_ref01_data_result:data_get() or segment_ref01_data_result)
    assert.is_not_nil(segment_ref01_data)
    assert.is_not_nil(segment_ref01_data["id"])

    -- LIST
    local segment_ref01_match = {}

    local segment_ref01_list_result, err = segment_ref01_ent:list(segment_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(segment_ref01_list_result)
    ...
    -- REMOVE
    local segment_ref01_match_rm0 = {
      id = segment_ref01_data["id"],
    }
    local _, err = segment_ref01_ent:remove(segment_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local segment_ref01_match_rt0 = {}

    local segment_ref01_list_rt0_result, err = segment_ref01_ent:list(segment_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(segment_ref01_list_rt0_result)
...
function segment_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("CUSTOMERIO_APP_TEST_SEGMENT_ENTID")
