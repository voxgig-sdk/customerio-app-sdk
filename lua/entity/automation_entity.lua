...
local AutomationEntity = {}
AutomationEntity.__index = AutomationEntity


function AutomationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AutomationEntity)
  self._name = "automation"
  self._client = client
...
function AutomationEntity:get_name()
  return self._name
end


function AutomationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AutomationEntity.new(self._client, opts)
end
...
function AutomationEntity:mark_deleted()
  self._deleted = true
end


function AutomationEntity:deleted()
  return true == self._deleted
end


function AutomationEntity:data_set(args)
  if args ~= nil then
...
function AutomationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AutomationEntity:match_set(args)
  if args ~= nil then
...
function AutomationEntity:match_get()
...
function AutomationEntity:stream(action, args, callopts)
...
---@param reqmatch AutomationLoadMatch
---@param ctrl? table
---@return Automation
---@return string? err
function AutomationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch AutomationListMatch
---@param ctrl? table
---@return Automation[]
---@return string? err
function AutomationEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata AutomationUpdateData
---@param ctrl? table
---@return Automation
---@return string? err
function AutomationEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AutomationEntity:_run_op(ctx, post_done)
  local utility = self._utility
