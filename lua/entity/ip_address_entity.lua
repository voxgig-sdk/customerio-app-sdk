-- CustomerioApp SDK IpAddress entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local IpAddressEntity = {}
IpAddressEntity.__index = IpAddressEntity


function IpAddressEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, IpAddressEntity)
...
function IpAddressEntity:get_name()
  return self._name
end


function IpAddressEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return IpAddressEntity.new(self._client, opts)
end
...
function IpAddressEntity:mark_deleted()
  self._deleted = true
end


function IpAddressEntity:deleted()
  return true == self._deleted
end


function IpAddressEntity:data_set(args)
  if args ~= nil then
...
function IpAddressEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function IpAddressEntity:match_set(args)
  if args ~= nil then
...
function IpAddressEntity:match_get()
...
function IpAddressEntity:stream(action, args, callopts)
...
function IpAddressEntity:_run_op(ctx, post_done)
  local utility = self._utility
