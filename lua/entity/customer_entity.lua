...
CustomerEntity.__index = CustomerEntity


function CustomerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CustomerEntity)
  self._name = "customer"
  self._client = client
...
function CustomerEntity:get_name()
  return self._name
end


function CustomerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CustomerEntity.new(self._client, opts)
end
...
function CustomerEntity:mark_deleted()
  self._deleted = true
end


function CustomerEntity:deleted()
  return true == self._deleted
end


function CustomerEntity:data_set(args)
  if args ~= nil then
...
function CustomerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CustomerEntity:match_set(args)
  if args ~= nil then
...
function CustomerEntity:match_get()
...
function CustomerEntity:stream(action, args, callopts)
...
---@return Customer[]
---@return string? err
function CustomerEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function CustomerEntity:_run_op(ctx, post_done)
  local utility = self._utility
