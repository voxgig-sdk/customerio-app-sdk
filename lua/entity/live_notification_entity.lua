...
local LiveNotificationEntity = {}
LiveNotificationEntity.__index = LiveNotificationEntity


function LiveNotificationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LiveNotificationEntity)
  self._name = "live_notification"
  self._client = client
...
function LiveNotificationEntity:get_name()
  return self._name
end


function LiveNotificationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LiveNotificationEntity.new(self._client, opts)
end
...
function LiveNotificationEntity:mark_deleted()
  self._deleted = true
end


function LiveNotificationEntity:deleted()
  return true == self._deleted
end


function LiveNotificationEntity:data_set(args)
  if args ~= nil then
...
function LiveNotificationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LiveNotificationEntity:match_set(args)
  if args ~= nil then
...
function LiveNotificationEntity:match_get()
...
function LiveNotificationEntity:stream(action, args, callopts)
...
---@param reqmatch LiveNotificationLoadMatch
---@param ctrl? table
---@return LiveNotification
---@return string? err
function LiveNotificationEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata LiveNotificationCreateData
---@param ctrl? table
---@return LiveNotification
---@return string? err
function LiveNotificationEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function LiveNotificationEntity:_run_op(ctx, post_done)
  local utility = self._utility
