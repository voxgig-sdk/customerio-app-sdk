...
local TransactionalEntity = {}
TransactionalEntity.__index = TransactionalEntity


function TransactionalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TransactionalEntity)
  self._name = "transactional"
  self._client = client
...
function TransactionalEntity:get_name()
  return self._name
end


function TransactionalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TransactionalEntity.new(self._client, opts)
end
...
function TransactionalEntity:mark_deleted()
  self._deleted = true
end


function TransactionalEntity:deleted()
  return true == self._deleted
end


function TransactionalEntity:data_set(args)
  if args ~= nil then
...
function TransactionalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TransactionalEntity:match_set(args)
  if args ~= nil then
...
function TransactionalEntity:match_get()
...
function TransactionalEntity:stream(action, args, callopts)
...
---@param reqmatch TransactionalLoadMatch
---@param ctrl? table
---@return Transactional
---@return string? err
function TransactionalEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch TransactionalListMatch
---@param ctrl? table
---@return Transactional[]
---@return string? err
function TransactionalEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata TransactionalUpdateData
---@param ctrl? table
---@return Transactional
---@return string? err
function TransactionalEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function TransactionalEntity:_run_op(ctx, post_done)
  local utility = self._utility
