-- CustomerioApp SDK ObjectType entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ObjectTypeEntity = {}
ObjectTypeEntity.__index = ObjectTypeEntity


function ObjectTypeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ObjectTypeEntity)
  self._name = "object_type"
  self._client = client
...
function ObjectTypeEntity:get_name()
  return self._name
end


function ObjectTypeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ObjectTypeEntity.new(self._client, opts)
end
...
function ObjectTypeEntity:mark_deleted()
  self._deleted = true
end


function ObjectTypeEntity:deleted()
  return true == self._deleted
end


function ObjectTypeEntity:data_set(args)
  if args ~= nil then
...
function ObjectTypeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ObjectTypeEntity:match_set(args)
  if args ~= nil then
...
function ObjectTypeEntity:match_get()
...
function ObjectTypeEntity:stream(action, args, callopts)
...
function ObjectTypeEntity:_run_op(ctx, post_done)
  local utility = self._utility
