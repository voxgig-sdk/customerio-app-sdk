-- CustomerioApp SDK SearchSuppression entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SearchSuppressionEntity = {}
SearchSuppressionEntity.__index = SearchSuppressionEntity


function SearchSuppressionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SearchSuppressionEntity)
  self._name = "search_suppression"
  self._client = client
...
function SearchSuppressionEntity:get_name()
  return self._name
end


function SearchSuppressionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SearchSuppressionEntity.new(self._client, opts)
end
...
function SearchSuppressionEntity:mark_deleted()
  self._deleted = true
end


function SearchSuppressionEntity:deleted()
  return true == self._deleted
end


function SearchSuppressionEntity:data_set(args)
  if args ~= nil then
...
function SearchSuppressionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SearchSuppressionEntity:match_set(args)
  if args ~= nil then
...
function SearchSuppressionEntity:match_get()
...
function SearchSuppressionEntity:stream(action, args, callopts)
...
function SearchSuppressionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return SearchSuppressionEntity
