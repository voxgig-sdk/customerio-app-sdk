-- CustomerioApp SDK Sms entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SmsEntity = {}
SmsEntity.__index = SmsEntity


function SmsEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SmsEntity)
  self._name = "sms"
  self._client = client
...
function SmsEntity:get_name()
  return self._name
end


function SmsEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SmsEntity.new(self._client, opts)
end
...
function SmsEntity:mark_deleted()
  self._deleted = true
end


function SmsEntity:deleted()
  return true == self._deleted
end


function SmsEntity:data_set(args)
  if args ~= nil then
...
function SmsEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SmsEntity:match_set(args)
  if args ~= nil then
...
function SmsEntity:stream(action, args, callopts)
...
function SmsEntity:_run_op(ctx, post_done)
  local utility = self._utility
