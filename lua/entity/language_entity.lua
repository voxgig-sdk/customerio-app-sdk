-- CustomerioApp SDK Language entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LanguageEntity = {}
LanguageEntity.__index = LanguageEntity


function LanguageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LanguageEntity)
  self._name = "language"
  self._client = client
...
function LanguageEntity:get_name()
  return self._name
end


function LanguageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LanguageEntity.new(self._client, opts)
end
...
function LanguageEntity:mark_deleted()
  self._deleted = true
end


function LanguageEntity:deleted()
  return true == self._deleted
end


function LanguageEntity:data_set(args)
  if args ~= nil then
...
function LanguageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LanguageEntity:match_set(args)
  if args ~= nil then
...
function LanguageEntity:match_get()
...
function LanguageEntity:stream(action, args, callopts)
...
function LanguageEntity:_run_op(ctx, post_done)
  local utility = self._utility
