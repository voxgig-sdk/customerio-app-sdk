-- CustomerioApp SDK Export entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ExportEntity = {}
ExportEntity.__index = ExportEntity


function ExportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ExportEntity)
  self._name = "export"
  self._client = client
...
function ExportEntity:get_name()
  return self._name
end


function ExportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ExportEntity.new(self._client, opts)
end
...
function ExportEntity:mark_deleted()
  self._deleted = true
end


function ExportEntity:deleted()
  return true == self._deleted
end


function ExportEntity:data_set(args)
  if args ~= nil then
...
function ExportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ExportEntity:match_set(args)
  if args ~= nil then
...
function ExportEntity:match_get()
...
function ExportEntity:stream(action, args, callopts)
...
---@param reqdata ExportCreateData
---@param ctrl? table
---@return Export
---@return string? err
function ExportEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ExportEntity:_run_op(ctx, post_done)
  local utility = self._utility
