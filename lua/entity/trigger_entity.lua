-- CustomerioApp SDK Trigger entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TriggerEntity = {}
TriggerEntity.__index = TriggerEntity


function TriggerEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TriggerEntity)
  self._name = "trigger"
  self._client = client
...
function TriggerEntity:get_name()
  return self._name
end


function TriggerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TriggerEntity.new(self._client, opts)
end
...
function TriggerEntity:mark_deleted()
  self._deleted = true
end


function TriggerEntity:deleted()
  return true == self._deleted
end


function TriggerEntity:data_set(args)
  if args ~= nil then
...
function TriggerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TriggerEntity:match_set(args)
  if args ~= nil then
...
function TriggerEntity:match_get()
...
function TriggerEntity:stream(action, args, callopts)
...
function TriggerEntity:_run_op(ctx, post_done)
  local utility = self._utility
