...
local EspSuppressionEntity = {}
EspSuppressionEntity.__index = EspSuppressionEntity


function EspSuppressionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EspSuppressionEntity)
  self._name = "esp_suppression"
  self._client = client
...
function EspSuppressionEntity:get_name()
  return self._name
end


function EspSuppressionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EspSuppressionEntity.new(self._client, opts)
end
...
function EspSuppressionEntity:mark_deleted()
  self._deleted = true
end


function EspSuppressionEntity:deleted()
  return true == self._deleted
end


function EspSuppressionEntity:data_set(args)
  if args ~= nil then
...
function EspSuppressionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EspSuppressionEntity:match_set(args)
  if args ~= nil then
...
function EspSuppressionEntity:match_get()
...
function EspSuppressionEntity:stream(action, args, callopts)
...
---@param reqmatch EspSuppressionLoadMatch
---@param ctrl? table
---@return EspSuppression
---@return string? err
function EspSuppressionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata EspSuppressionCreateData
---@param ctrl? table
---@return EspSuppression
---@return string? err
function EspSuppressionEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch EspSuppressionRemoveMatch
---@param ctrl? table
---@return EspSuppression
---@return string? err
function EspSuppressionEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function EspSuppressionEntity:_run_op(ctx, post_done)
  local utility = self._utility
