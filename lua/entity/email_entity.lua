-- CustomerioApp SDK Email entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EmailEntity = {}
EmailEntity.__index = EmailEntity


function EmailEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmailEntity)
  self._name = "email"
  self._client = client
...
function EmailEntity:get_name()
  return self._name
end


function EmailEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmailEntity.new(self._client, opts)
end
...
function EmailEntity:mark_deleted()
  self._deleted = true
end


function EmailEntity:deleted()
  return true == self._deleted
end


function EmailEntity:data_set(args)
  if args ~= nil then
...
function EmailEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmailEntity:match_set(args)
  if args ~= nil then
...
function EmailEntity:match_get()
...
function EmailEntity:stream(action, args, callopts)
...
function EmailEntity:_run_op(ctx, post_done)
  local utility = self._utility
