-- CustomerioApp SDK InboxMessage entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InboxMessageEntity = {}
InboxMessageEntity.__index = InboxMessageEntity


function InboxMessageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InboxMessageEntity)
  self._name = "inbox_message"
  self._client = client
...
function InboxMessageEntity:get_name()
  return self._name
end


function InboxMessageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InboxMessageEntity.new(self._client, opts)
end
...
function InboxMessageEntity:mark_deleted()
  self._deleted = true
end


function InboxMessageEntity:deleted()
  return true == self._deleted
end


function InboxMessageEntity:data_set(args)
  if args ~= nil then
...
function InboxMessageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InboxMessageEntity:match_set(args)
  if args ~= nil then
...
function InboxMessageEntity:match_get()
...
function InboxMessageEntity:stream(action, args, callopts)
...
function InboxMessageEntity:_run_op(ctx, post_done)
  local utility = self._utility
