...
WorkspaceEntity.__index = WorkspaceEntity


function WorkspaceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WorkspaceEntity)
...
function WorkspaceEntity:get_name()
  return self._name
end


function WorkspaceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WorkspaceEntity.new(self._client, opts)
end
...
function WorkspaceEntity:mark_deleted()
  self._deleted = true
end


function WorkspaceEntity:deleted()
  return true == self._deleted
end


function WorkspaceEntity:data_set(args)
  if args ~= nil then
...
function WorkspaceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WorkspaceEntity:match_set(args)
  if args ~= nil then
...
function WorkspaceEntity:match_get()
...
function WorkspaceEntity:stream(action, args, callopts)
...
---@param reqmatch WorkspaceListMatch
---@param ctrl? table
---@return Workspace[]
---@return string? err
function WorkspaceEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function WorkspaceEntity:_run_op(ctx, post_done)
  local utility = self._utility
