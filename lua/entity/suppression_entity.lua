-- CustomerioApp SDK Suppression entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local SuppressionEntity = {}
SuppressionEntity.__index = SuppressionEntity


function SuppressionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SuppressionEntity)
  self._name = "suppression"
  self._client = client
...
function SuppressionEntity:get_name()
  return self._name
end


function SuppressionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SuppressionEntity.new(self._client, opts)
end
...
function SuppressionEntity:mark_deleted()
  self._deleted = true
end


function SuppressionEntity:deleted()
  return true == self._deleted
end


function SuppressionEntity:data_set(args)
  if args ~= nil then
...
function SuppressionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SuppressionEntity:match_set(args)
  if args ~= nil then
...
function SuppressionEntity:match_get()
...
function SuppressionEntity:stream(action, args, callopts)
...
function SuppressionEntity:_run_op(ctx, post_done)
  local utility = self._utility
