-- CustomerioApp SDK Relationship entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RelationshipEntity = {}
RelationshipEntity.__index = RelationshipEntity


function RelationshipEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RelationshipEntity)
...
function RelationshipEntity:get_name()
  return self._name
end


function RelationshipEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RelationshipEntity.new(self._client, opts)
end
...
function RelationshipEntity:mark_deleted()
  self._deleted = true
end


function RelationshipEntity:deleted()
  return true == self._deleted
end


function RelationshipEntity:data_set(args)
  if args ~= nil then
...
function RelationshipEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RelationshipEntity:match_set(args)
  if args ~= nil then
...
function RelationshipEntity:match_get()
...
function RelationshipEntity:stream(action, args, callopts)
...
function RelationshipEntity:_run_op(ctx, post_done)
  local utility = self._utility
