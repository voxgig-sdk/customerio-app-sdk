...
local SegmentEntity = {}
SegmentEntity.__index = SegmentEntity


function SegmentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SegmentEntity)
  self._name = "segment"
  self._client = client
...
function SegmentEntity:get_name()
  return self._name
end


function SegmentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SegmentEntity.new(self._client, opts)
end
...
function SegmentEntity:mark_deleted()
  self._deleted = true
end


function SegmentEntity:deleted()
  return true == self._deleted
end


function SegmentEntity:data_set(args)
  if args ~= nil then
...
function SegmentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SegmentEntity:match_set(args)
  if args ~= nil then
...
function SegmentEntity:match_get()
...
function SegmentEntity:stream(action, args, callopts)
...
---@param reqmatch SegmentLoadMatch
---@param ctrl? table
---@return Segment
---@return string? err
function SegmentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SegmentListMatch
---@param ctrl? table
---@return Segment[]
---@return string? err
function SegmentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
---@param reqdata SegmentCreateData
---@param ctrl? table
---@return Segment
---@return string? err
function SegmentEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqmatch SegmentRemoveMatch
---@param ctrl? table
---@return Segment
---@return string? err
function SegmentEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function SegmentEntity:_run_op(ctx, post_done)
  local utility = self._utility
