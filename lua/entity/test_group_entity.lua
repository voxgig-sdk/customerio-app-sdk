-- CustomerioApp SDK TestGroup entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TestGroupEntity = {}
TestGroupEntity.__index = TestGroupEntity


function TestGroupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TestGroupEntity)
...
function TestGroupEntity:get_name()
  return self._name
end


function TestGroupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TestGroupEntity.new(self._client, opts)
end
...
function TestGroupEntity:mark_deleted()
  self._deleted = true
end


function TestGroupEntity:deleted()
  return true == self._deleted
end


function TestGroupEntity:data_set(args)
  if args ~= nil then
...
function TestGroupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TestGroupEntity:match_set(args)
  if args ~= nil then
...
function TestGroupEntity:match_get()
...
function TestGroupEntity:stream(action, args, callopts)
...
function TestGroupEntity:_run_op(ctx, post_done)
  local utility = self._utility
