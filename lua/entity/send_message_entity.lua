...
local SendMessageEntity = {}
SendMessageEntity.__index = SendMessageEntity


function SendMessageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SendMessageEntity)
...
function SendMessageEntity:get_name()
  return self._name
end


function SendMessageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SendMessageEntity.new(self._client, opts)
end
...
function SendMessageEntity:mark_deleted()
  self._deleted = true
end


function SendMessageEntity:deleted()
  return true == self._deleted
end


function SendMessageEntity:data_set(args)
  if args ~= nil then
...
function SendMessageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SendMessageEntity:match_set(args)
  if args ~= nil then
...
function SendMessageEntity:match_get()
...
function SendMessageEntity:stream(action, args, callopts)
...
---@param reqdata SendMessageCreateData
---@param ctrl? table
---@return SendMessage
---@return string? err
function SendMessageEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function SendMessageEntity:_run_op(ctx, post_done)
  local utility = self._utility
