...
local ImportEntity = {}
ImportEntity.__index = ImportEntity


function ImportEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ImportEntity)
  self._name = "import"
  self._client = client
...
function ImportEntity:get_name()
  return self._name
end


function ImportEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ImportEntity.new(self._client, opts)
end
...
function ImportEntity:mark_deleted()
  self._deleted = true
end


function ImportEntity:deleted()
  return true == self._deleted
end


function ImportEntity:data_set(args)
  if args ~= nil then
...
function ImportEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ImportEntity:match_set(args)
  if args ~= nil then
...
function ImportEntity:match_get()
...
function ImportEntity:stream(action, args, callopts)
...
---@param reqmatch ImportLoadMatch
---@param ctrl? table
---@return Import
---@return string? err
function ImportEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata ImportCreateData
---@param ctrl? table
---@return Import
---@return string? err
function ImportEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function ImportEntity:_run_op(ctx, post_done)
  local utility = self._utility
