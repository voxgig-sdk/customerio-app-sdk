-- CustomerioApp SDK Attribute entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AttributeEntity = {}
AttributeEntity.__index = AttributeEntity


function AttributeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AttributeEntity)
...
function AttributeEntity:get_name()
  return self._name
end


function AttributeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AttributeEntity.new(self._client, opts)
end
...
function AttributeEntity:mark_deleted()
  self._deleted = true
end


function AttributeEntity:deleted()
  return true == self._deleted
end


function AttributeEntity:data_set(args)
  if args ~= nil then
...
function AttributeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AttributeEntity:match_set(args)
  if args ~= nil then
...
function AttributeEntity:match_get()
...
function AttributeEntity:stream(action, args, callopts)
...
function AttributeEntity:_run_op(ctx, post_done)
  local utility = self._utility
