-- CustomerioApp SDK Link entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LinkEntity = {}
LinkEntity.__index = LinkEntity


function LinkEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LinkEntity)
  self._name = "link"
  self._client = client
...
function LinkEntity:get_name()
  return self._name
end


function LinkEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LinkEntity.new(self._client, opts)
end
...
function LinkEntity:mark_deleted()
  self._deleted = true
end


function LinkEntity:deleted()
  return true == self._deleted
end


function LinkEntity:data_set(args)
  if args ~= nil then
...
function LinkEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LinkEntity:match_set(args)
  if args ~= nil then
...
function LinkEntity:stream(action, args, callopts)
...
function LinkEntity:_run_op(ctx, post_done)
  local utility = self._utility
