-- CustomerioApp SDK Campaign entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CampaignEntity = {}
CampaignEntity.__index = CampaignEntity


function CampaignEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CampaignEntity)
  self._name = "campaign"
  self._client = client
...
function CampaignEntity:get_name()
  return self._name
end


function CampaignEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CampaignEntity.new(self._client, opts)
end
...
function CampaignEntity:mark_deleted()
  self._deleted = true
end


function CampaignEntity:deleted()
  return true == self._deleted
end


function CampaignEntity:data_set(args)
  if args ~= nil then
...
function CampaignEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CampaignEntity:match_set(args)
  if args ~= nil then
...
function CampaignEntity:match_get()
...
function CampaignEntity:stream(action, args, callopts)
...
function CampaignEntity:_run_op(ctx, post_done)
  local utility = self._utility
