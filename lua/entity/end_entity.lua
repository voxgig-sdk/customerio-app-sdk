-- CustomerioApp SDK End entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local EndEntity = {}
EndEntity.__index = EndEntity


function EndEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EndEntity)
  self._name = "end"
  self._client = client
...
function EndEntity:get_name()
  return self._name
end


function EndEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EndEntity.new(self._client, opts)
end
...
function EndEntity:mark_deleted()
  self._deleted = true
end


function EndEntity:deleted()
  return true == self._deleted
end


function EndEntity:data_set(args)
  if args ~= nil then
...
function EndEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EndEntity:match_set(args)
  if args ~= nil then
...
function EndEntity:stream(action, args, callopts)
...
function EndEntity:_run_op(ctx, post_done)
  local utility = self._utility
