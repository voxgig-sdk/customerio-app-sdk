...
local AssetEntity = {}
AssetEntity.__index = AssetEntity


function AssetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AssetEntity)
  self._name = "asset"
  self._client = client
...
function AssetEntity:get_name()
  return self._name
end


function AssetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AssetEntity.new(self._client, opts)
end
...
function AssetEntity:mark_deleted()
  self._deleted = true
end


function AssetEntity:deleted()
  return true == self._deleted
end


function AssetEntity:data_set(args)
  if args ~= nil then
...
function AssetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AssetEntity:match_set(args)
  if args ~= nil then
...
function AssetEntity:match_get()
...
function AssetEntity:stream(action, args, callopts)
...
---@param reqdata AssetUpdateData
---@param ctrl? table
---@return Asset
---@return string? err
function AssetEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function AssetEntity:_run_op(ctx, post_done)
  local utility = self._utility
