...
local OptOutEntity = {}
OptOutEntity.__index = OptOutEntity


function OptOutEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OptOutEntity)
  self._name = "opt_out"
  self._client = client
...
function OptOutEntity:get_name()
  return self._name
end


function OptOutEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OptOutEntity.new(self._client, opts)
end
...
function OptOutEntity:mark_deleted()
  self._deleted = true
end


function OptOutEntity:deleted()
  return true == self._deleted
end


function OptOutEntity:data_set(args)
  if args ~= nil then
...
function OptOutEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OptOutEntity:match_set(args)
  if args ~= nil then
...
function OptOutEntity:match_get()
...
function OptOutEntity:stream(action, args, callopts)
...
---@param reqmatch OptOutListMatch
---@param ctrl? table
---@return OptOut[]
---@return string? err
function OptOutEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata OptOutUpdateData
---@param ctrl? table
---@return OptOut
---@return string? err
function OptOutEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
function OptOutEntity:_run_op(ctx, post_done)
  local utility = self._utility
