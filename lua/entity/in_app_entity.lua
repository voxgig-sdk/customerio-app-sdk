-- CustomerioApp SDK InApp entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InAppEntity = {}
InAppEntity.__index = InAppEntity


function InAppEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InAppEntity)
  self._name = "in_app"
  self._client = client
...
function InAppEntity:get_name()
  return self._name
end


function InAppEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InAppEntity.new(self._client, opts)
end
...
function InAppEntity:mark_deleted()
  self._deleted = true
end


function InAppEntity:deleted()
  return true == self._deleted
end


function InAppEntity:data_set(args)
  if args ~= nil then
...
function InAppEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InAppEntity:match_set(args)
  if args ~= nil then
...
function InAppEntity:match_get()
...
function InAppEntity:stream(action, args, callopts)
...
function InAppEntity:_run_op(ctx, post_done)
  local utility = self._utility
