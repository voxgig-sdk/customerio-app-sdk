...
local SnippetEntity = {}
SnippetEntity.__index = SnippetEntity


function SnippetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SnippetEntity)
  self._name = "snippet"
  self._client = client
...
function SnippetEntity:get_name()
  return self._name
end


function SnippetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SnippetEntity.new(self._client, opts)
end
...
function SnippetEntity:mark_deleted()
  self._deleted = true
end


function SnippetEntity:deleted()
  return true == self._deleted
end


function SnippetEntity:data_set(args)
  if args ~= nil then
...
function SnippetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SnippetEntity:match_set(args)
  if args ~= nil then
...
function SnippetEntity:match_get()
...
function SnippetEntity:stream(action, args, callopts)
...
---@param reqmatch SnippetListMatch
---@param ctrl? table
---@return Snippet[]
---@return string? err
function SnippetEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata SnippetCreateData
---@param ctrl? table
---@return Snippet
---@return string? err
function SnippetEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
---@param reqdata SnippetUpdateData
---@param ctrl? table
---@return Snippet
---@return string? err
function SnippetEntity:update(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "update",
    ctrl = ctrl,
...
---@param reqmatch SnippetRemoveMatch
---@param ctrl? table
---@return Snippet
---@return string? err
function SnippetEntity:remove(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "remove",
    ctrl = ctrl,
...
function SnippetEntity:_run_op(ctx, post_done)
  local utility = self._utility
