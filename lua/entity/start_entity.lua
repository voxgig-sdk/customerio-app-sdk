-- CustomerioApp SDK Start entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local StartEntity = {}
StartEntity.__index = StartEntity


function StartEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StartEntity)
  self._name = "start"
  self._client = client
...
function StartEntity:get_name()
  return self._name
end


function StartEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StartEntity.new(self._client, opts)
end
...
function StartEntity:mark_deleted()
  self._deleted = true
end


function StartEntity:deleted()
  return true == self._deleted
end


function StartEntity:data_set(args)
  if args ~= nil then
...
function StartEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StartEntity:match_set(args)
  if args ~= nil then
...
function StartEntity:match_get()
...
function StartEntity:stream(action, args, callopts)
...
function StartEntity:_run_op(ctx, post_done)
  local utility = self._utility
