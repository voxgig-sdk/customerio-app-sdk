...
ActivityEntity.__index = ActivityEntity


function ActivityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ActivityEntity)
  self._name = "activity"
  self._client = client
...
function ActivityEntity:get_name()
  return self._name
end


function ActivityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ActivityEntity.new(self._client, opts)
end
...
function ActivityEntity:mark_deleted()
  self._deleted = true
end


function ActivityEntity:deleted()
  return true == self._deleted
end


function ActivityEntity:data_set(args)
  if args ~= nil then
...
function ActivityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ActivityEntity:match_set(args)
  if args ~= nil then
...
function ActivityEntity:match_get()
...
function ActivityEntity:stream(action, args, callopts)
...
---@param reqmatch ActivityListMatch
---@param ctrl? table
---@return Activity[]
---@return string? err
function ActivityEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ActivityEntity:_run_op(ctx, post_done)
  local utility = self._utility
