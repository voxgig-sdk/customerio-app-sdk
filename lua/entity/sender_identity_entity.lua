...
local SenderIdentityEntity = {}
SenderIdentityEntity.__index = SenderIdentityEntity


function SenderIdentityEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SenderIdentityEntity)
  self._name = "sender_identity"
  self._client = client
...
function SenderIdentityEntity:get_name()
  return self._name
end


function SenderIdentityEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SenderIdentityEntity.new(self._client, opts)
end
...
function SenderIdentityEntity:mark_deleted()
  self._deleted = true
end


function SenderIdentityEntity:deleted()
  return true == self._deleted
end


function SenderIdentityEntity:data_set(args)
  if args ~= nil then
...
function SenderIdentityEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SenderIdentityEntity:match_set(args)
  if args ~= nil then
...
function SenderIdentityEntity:match_get()
...
function SenderIdentityEntity:stream(action, args, callopts)
...
---@param reqmatch SenderIdentityLoadMatch
---@param ctrl? table
---@return SenderIdentity
---@return string? err
function SenderIdentityEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch SenderIdentityListMatch
---@param ctrl? table
---@return SenderIdentity[]
---@return string? err
function SenderIdentityEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function SenderIdentityEntity:_run_op(ctx, post_done)
  local utility = self._utility
