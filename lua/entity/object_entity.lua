-- CustomerioApp SDK Object entity
...
function ObjectEntity.new(client, entopts)
  entopts = entopts or {}
...
function ObjectEntity:get_name()
  return self._name
end


function ObjectEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ObjectEntity.new(self._client, opts)
end
...
function ObjectEntity:mark_deleted()
  self._deleted = true
end


function ObjectEntity:deleted()
  return true == self._deleted
end


function ObjectEntity:data_set(args)
  if args ~= nil then
...
function ObjectEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ObjectEntity:match_set(args)
  if args ~= nil then
...
function ObjectEntity:match_get()
...
function ObjectEntity:stream(action, args, callopts)
...
function ObjectEntity:_run_op(ctx, post_done)
  local utility = self._utility
