...
(0, node_test_1.describe)('MessageDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-message', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-message', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'CUSTOMERIO_APP_TEST_MESSAGE_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['CUSTOMERIO_APP_TEST_MESSAGE_ENTID'];
