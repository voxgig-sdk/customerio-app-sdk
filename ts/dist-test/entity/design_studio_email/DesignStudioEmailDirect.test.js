...
(0, node_test_1.describe)('DesignStudioEmailDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-design_studio_email', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-design_studio_email', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["client01", "email01", "inbox_preview01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.client_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.client_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-design_studio_email', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-design_studio_email', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CUSTOMERIO_APP_TEST_DESIGN_STUDIO_EMAIL_ENTID'];
...
//# sourceMappingURL=DesignStudioEmailDirect.test.js.map