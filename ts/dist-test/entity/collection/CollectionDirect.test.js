...
(0, node_test_1.describe)('CollectionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-collection', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-collection', setup.live))
            return;
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CUSTOMERIO_APP_TEST_COLLECTION_ENTID'];
