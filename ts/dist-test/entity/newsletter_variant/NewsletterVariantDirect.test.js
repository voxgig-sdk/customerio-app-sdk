...
(0, node_test_1.describe)('NewsletterVariantDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.CustomerioAppSDK({
    ...
    (0, node_test_1.test)('direct-load-newsletter_variant', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-newsletter_variant', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["language01", "newsletter01", "test_group01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                method: 'GET',
                params: {
                    id: setup.idmap['newsletter_variant01'],
                },
            });
            ...
            const candidateId = listArr[0]?.language ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.language = candidateId;
    ...
    (0, node_test_1.test)('direct-list-newsletter_variant', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-newsletter_variant', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["newsletter_variant01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CUSTOMERIO_APP_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CustomerioAppSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.CUSTOMERIO_APP_APIKEY,
        }));
        let idmap = env['CUSTOMERIO_APP_TEST_NEWSLETTER_VARIANT_ENTID'];
    ...
    const client = new __1.CustomerioAppSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=NewsletterVariantDirect.test.js.map