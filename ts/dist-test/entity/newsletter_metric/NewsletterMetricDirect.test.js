...
(0, node_test_1.describe)('NewsletterMetricDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-newsletter_metric', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-newsletter_metric', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["content01", "newsletter01"]))
            return;
        ...
        if (setup.live) {
            const listResult = await client.direct({
                ...
                method: 'GET',
                params: {
                    id: setup.idmap['newsletter_metric01'],
                },
            });
            ...
            const candidateId = listArr[0]?.content_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.content_id = candidateId;
    ...
    (0, node_test_1.test)('direct-list-newsletter_metric', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-newsletter_metric', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["newsletter_metric01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CUSTOMERIO_APP_TEST_NEWSLETTER_METRIC_ENTID'];
...
//# sourceMappingURL=NewsletterMetricDirect.test.js.map