# CustomerioApp TypeScript SDK Reference

Complete API reference for the CustomerioApp TypeScript SDK.
...
| `customer_identifiers` | `Record<string, any>` | Yes | Identifiers for the person in a response—`id`, `cio_id`, and `email`. |
| `data` | `any` | No |  |
...
| Action | Route | Call |
| --- | --- | --- |
| `file` | `/v1/assets/files` | `client.Asset().create({ $action: 'file', ... })` |
| `folder` | `/v1/assets/folders` | `client.Asset().create({ $action: 'folder', ... })` |
| `folder` | `/v1/assets/folders` | `client.Asset().list({ $action: 'folder', ... })` |
...
| `language_variants` | `Record<string, any>` | No | Metrics for each language variant of a multi-language message, keyed by the variant's action ID. |
...
| `metric` | `Record<string, any>` | No | Contains metrics for the link. |
| `metrics` | `Record<string, any>` | No | Metrics for an individual instance of a message; each item in the object represents the timestamp when a message achieved a particular metric. |
...
| `series` | `Record<string, any>` | No | Metrics grouped by the requested resolution. |
...
| `tracked_responses` | `Record<string, any>` | No | Tracked in-app survey responses, keyed by response option name. |
...
| Action | Route | Call |
| --- | --- | --- |
| `action` | `/v1/broadcasts/{broadcast_id}/actions` | `client.Broadcast().list({ $action: 'action', ... })` |
| `message` | `/v1/broadcasts/{broadcast_id}/messages` | `client.Broadcast().list({ $action: 'message', ... })` |
| `metric_link` | `/v1/broadcasts/{broadcast_id}/metrics/links` | `client.Broadcast().list({ $action: 'metric_link', ... })` |
| `trigger` | `/v1/broadcasts/{broadcast_id}/triggers` | `client.Broadcast().list({ $action: 'trigger', ... })` |
| `metric` | `/v1/broadcasts/{broadcast_id}/metrics` | `client.Broadcast().load({ $action: 'metric', ... })` |

An action returns that action's OWN response, which is not necessarily a
Broadcast record — check the API definition for its shape.

```ts
const result = await client.Broadcast().list({
  $action: 'action',
  /* ...the action's own arguments */
})
```
...
| Action | Route | Call |
| --- | --- | --- |
| `content` | `/v1/collections/{collection_id}/content` | `client.Collection().load({ $action: 'content', ... })` |
| `content` | `/v1/collections/{collection_id}/content` | `client.Collection().update({ $action: 'content', ... })` |

An action returns that action's OWN response, which is not necessarily a
Collection record — check the API definition for its shape.

```ts
const result = await client.Collection().load({
  $action: 'content',
  /* ...the action's own arguments */
})
```
...
| Action | Route | Call |
| --- | --- | --- |
| `attribute` | `/v1/customers/attributes` | `client.Customer().create({ $action: 'attribute', ... })` |
| `activity` | `/v1/customers/{customer_id}/activities` | `client.Customer().list({ $action: 'activity', ... })` |
| `message` | `/v1/customers/{customer_id}/messages` | `client.Customer().list({ $action: 'message', ... })` |
| `relationship` | `/v1/customers/{customer_id}/relationships` | `client.Customer().list({ $action: 'relationship', ... })` |
| `segment` | `/v1/customers/{customer_id}/segments` | `client.Customer().list({ $action: 'segment', ... })` |
| `attribute` | `/v1/customers/{customer_id}/attributes` | `client.Customer().load({ $action: 'attribute', ... })` |
| `subscription_preference` | `/v1/customers/{customer_id}/subscription_preferences` | `client.Customer().load({ $action: 'subscription_preference', ... })` |
...
```ts
const results = await client.Customer().list({ email: "example" })
```
...
| Action | Route | Call |
| --- | --- | --- |
| `attribute` | `/v1/data_index/attributes` | `client.DataIndex().create({ $action: 'attribute', ... })` |
| `event` | `/v1/data_index/events` | `client.DataIndex().create({ $action: 'event', ... })` |

An action returns that action's OWN response, which is not necessarily a
DataIndex record — check the API definition for its shape.
...
| Action | Route | Call |
| --- | --- | --- |
| `component` | `/v1/design_studio/components` | `client.DesignStudio().create({ $action: 'component', ... })` |
| `folder` | `/v1/design_studio/folders` | `client.DesignStudio().create({ $action: 'folder', ... })` |
| `component` | `/v1/design_studio/components` | `client.DesignStudio().list({ $action: 'component', ... })` |
| `folder` | `/v1/design_studio/folders` | `client.DesignStudio().list({ $action: 'folder', ... })` |

An action returns that action's OWN response, which is not necessarily a
DesignStudio record — check the API definition for its shape.
...
| `content` | `Record<string, any>` | No | The content of your email. |
...
| `emails` | `any[]` | No |  |
| `envelope` | `Record<string, any>` | No | The envelope of your email, like from and to addresses. |
...
| `node` | `Record<string, any>` | No | The content and settings stored in the version. |
...
| `sample_data` | `Record<string, any>` | No | Liquid variables to render with, as a JSON object—any shape is accepted, from flat variables like `{"first_name": "Janine"}` to nested ones like `{"customer": {"first_name": "Janine"}}`. |
...
| `transformers` | `Record<string, any>` | No | Automate repetitive actions like removing white space and inlining CSS with [transformers](/journeys/design-studio/emails/code-editor/overview/#transformers). |
...
| Action | Route | Call |
| --- | --- | --- |
| `link` | `/v1/design_studio/emails/{id}/link` | `client.DesignStudioEmail().create({ $action: 'link', ... })` |
| `preview` | `/v1/design_studio/emails/{id}/preview` | `client.DesignStudioEmail().create({ $action: 'preview', ... })` |
| `publish` | `/v1/design_studio/emails/{id}/publish` | `client.DesignStudioEmail().create({ $action: 'publish', ... })` |
| `restore` | `/v1/design_studio/emails/{id}/versions/{version_id}/restore` | `client.DesignStudioEmail().create({ $action: 'restore', ... })` |

An action returns that action's OWN response, which is not necessarily a
DesignStudioEmail record — check the API definition for its shape.
...
| Action | Route | Call |
| --- | --- | --- |
| `customer` | `/v1/exports/customers` | `client.Export().create({ $action: 'customer', ... })` |
| `delivery` | `/v1/exports/deliveries` | `client.Export().create({ $action: 'delivery', ... })` |
| `download` | `/v1/exports/{export_id}/download` | `client.Export().load({ $action: 'download', ... })` |
...
| Action | Route | Call |
| --- | --- | --- |
| `ip_address` | `/v1/info/ip_addresses` | `client.Info().list({ $action: 'ip_address', ... })` |

An action returns that action's OWN response, which is not necessarily a
Info record — check the API definition for its shape.
...
| Action | Route | Call |
| --- | --- | --- |
| `end` | `/v1/live_notifications/end` | `client.LiveNotification().create({ $action: 'end', ... })` |
| `start` | `/v1/live_notifications/start` | `client.LiveNotification().create({ $action: 'start', ... })` |
| `update` | `/v1/live_notifications/update` | `client.LiveNotification().create({ $action: 'update', ... })` |

An action returns that action's OWN response, which is not necessarily a
LiveNotification record — check the API definition for its shape.
...
| Action | Route | Call |
| --- | --- | --- |
| `archived_message` | `/v1/messages/{message_id}/archived_message` | `client.Message().load({ $action: 'archived_message', ... })` |
...
| Action | Route | Call |
| --- | --- | --- |
| `schedule` | `/v1/newsletters/{newsletter_id}/schedule` | `client.Newsletter().create({ $action: 'schedule', ... })` |
| `send` | `/v1/newsletters/{newsletter_id}/send` | `client.Newsletter().create({ $action: 'send', ... })` |

An action returns that action's OWN response, which is not necessarily a
Newsletter record — check the API definition for its shape.
...
| Action | Route | Call |
| --- | --- | --- |
| `messages` | `/v1/newsletters/{newsletter_id}/messages` | `client.NewsletterMetric().list({ $action: 'messages', ... })` |

An action returns that action's OWN response, which is not necessarily a
NewsletterMetric record — check the API definition for its shape.

```ts
const result = await client.NewsletterMetric().list({
  $action: 'messages',
  /* ...the action's own arguments */
})
```
...
```ts
const results = await client.NewsletterMetric().list({ newsletter_id: 1 })
```
...
| Action | Route | Call |
| --- | --- | --- |
| `language` | `/v1/newsletters/{newsletter_id}/language` | `client.NewsletterVariant().create({ $action: 'language', ... })` |
| `test_groups` | `/v1/newsletters/{newsletter_id}/test_groups` | `client.NewsletterVariant().create({ $action: 'test_groups', ... })` |
| `contents` | `/v1/newsletters/{newsletter_id}/contents` | `client.NewsletterVariant().list({ $action: 'contents', ... })` |
| `test_groups` | `/v1/newsletters/{newsletter_id}/test_groups` | `client.NewsletterVariant().list({ $action: 'test_groups', ... })` |

An action returns that action's OWN response, which is not necessarily a
NewsletterVariant record — check the API definition for its shape.
...
```ts
const results = await client.NewsletterVariant().list({ id: 1 })
```
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `attributes` | `Record<string, any>` | No | Attributes assigned to this object. |
...
| `identifiers` | `Record<string, any>` | No | Identifies an object. |
...
| `timestamps` | `Record<string, any>` | No | The epoch timestamps when corresponding attributes were set on the object. |
...
| `tags` | `any[] | null` | No | The tags assigned to the segment, if any. |
...
| Action | Route | Call |
| --- | --- | --- |
| `customer_count` | `/v1/segments/{segment_id}/customer_count` | `client.Segment().load({ $action: 'customer_count', ... })` |
| `membership` | `/v1/segments/{segment_id}/membership` | `client.Segment().load({ $action: 'membership', ... })` |
| `used_by` | `/v1/segments/{segment_id}/used_by` | `client.Segment().load({ $action: 'used_by', ... })` |

An action returns that action's OWN response, which is not necessarily a
Segment record — check the API definition for its shape.

```ts
const result = await client.Segment().load({
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `attachments` | `Record<string, any>` | No | A dictionary of attachments where the filename is the key and the value is the base64-encoded contents. |
...
| `custom_data` | `Record<string, any>` | No | Optional key/value pairs you want to attach to the push payload. |
| `custom_device` | `any` | Yes | A device to perform an upsert operation at the time of send. |
| `custom_payload` | `Record<string, any>` | No | Optional key/value pairs you want to attach to the push payload. |
...
| `message_data` | `Record<string, any>` | No | An object containing the key-value pairs referenced using liquid in your message. |
...
| Action | Route | Call |
| --- | --- | --- |
| `used_by` | `/v1/sender_identities/{sender_id}/used_by` | `client.SenderIdentity().list({ $action: 'used_by', ... })` |

An action returns that action's OWN response, which is not necessarily a
SenderIdentity record — check the API definition for its shape.

```ts
const result = await client.SenderIdentity().list({
  $action: 'used_by',
  /* ...the action's own arguments */
})
```
...
| Action | Route | Call |
| --- | --- | --- |
| `token` | `/v1/subscription_center/{customer_id}/token` | `client.SubscriptionCenter().load({ $action: 'token', ... })` |

An action returns that action's OWN response, which is not necessarily a
SubscriptionCenter record — check the API definition for its shape.

```ts
const result = await client.SubscriptionCenter().load({
  $action: 'token',
  /* ...the action's own arguments */
})
```
...
| Action | Route | Call |
| --- | --- | --- |
| `content` | `/v1/transactional/{transactional_id}/contents` | `client.Transactional().list({ $action: 'content', ... })` |
| `message` | `/v1/transactional/{transactional_id}/messages` | `client.Transactional().list({ $action: 'message', ... })` |
| `metric_link` | `/v1/transactional/{transactional_id}/metrics/links` | `client.Transactional().list({ $action: 'metric_link', ... })` |
| `metric` | `/v1/transactional/{transactional_id}/metrics` | `client.Transactional().load({ $action: 'metric', ... })` |

An action returns that action's OWN response, which is not necessarily a
Transactional record — check the API definition for its shape.

```ts
const result = await client.Transactional().list({
  $action: 'content',
  /* ...the action's own arguments */
})
```
