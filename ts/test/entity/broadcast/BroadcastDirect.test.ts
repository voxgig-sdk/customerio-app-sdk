...
describe('BroadcastDirect', async () => {
  ...
  test('direct-load-broadcast', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-broadcast', setup.live)) return
    if (skipIfMissingIds(t, setup, ["action01"])) return
  ...
  test('direct-list-broadcast', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-broadcast', setup.live)) return
    if (skipIfMissingIds(t, setup, ["broadcast01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_BROADCAST_ENTID']
