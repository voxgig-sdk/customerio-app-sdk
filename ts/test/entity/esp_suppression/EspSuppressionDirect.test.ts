...
describe('EspSuppressionDirect', async () => {
  ...
  test('direct-load-esp_suppression', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-esp_suppression', setup.live)) return
    if (skipIfMissingIds(t, setup, ["domain_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_ESP_SUPPRESSION_ENTID']
