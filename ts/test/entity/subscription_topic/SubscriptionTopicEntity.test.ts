...
describe('SubscriptionTopicEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of []) {
      if (!live && maybeSkipControl(t, 'entityOp', 'subscription_topic.' + op, live)) return
    }
    ...
    let subscription_topic_ref01_data = Object.values(setup.data.existing.subscription_topic)[0] as any
