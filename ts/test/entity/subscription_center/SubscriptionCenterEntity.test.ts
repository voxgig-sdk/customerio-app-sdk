...
describe('SubscriptionCenterEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'subscription_center.' + op, live)) return
    }
    ...
    let subscription_center_ref01_data = Object.values(setup.data.existing.subscription_center)[0] as any

    // LIST
    const subscription_center_ref01_ent = client.SubscriptionCenter()
    const subscription_center_ref01_match: any = {}

    const subscription_center_ref01_list = (await subscription_center_ref01_ent.list(subscription_center_ref01_match)).map((e: any) => e.data())


    // LOAD
    const subscription_center_ref01_match_dt0: any = {}
