...
describe('NewsletterVariantDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CustomerioAppSDK({
  ...
  test('direct-load-newsletter_variant', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-newsletter_variant', setup.live)) return
    if (skipIfMissingIds(t, setup, ["language01","newsletter01","test_group01"])) return
  ...
  test('direct-list-newsletter_variant', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-newsletter_variant', setup.live)) return
    if (skipIfMissingIds(t, setup, ["newsletter_variant01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_NEWSLETTER_VARIANT_ENTID']
