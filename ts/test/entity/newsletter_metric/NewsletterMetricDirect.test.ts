...
describe('NewsletterMetricDirect', async () => {
  ...
  test('direct-load-newsletter_metric', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-newsletter_metric', setup.live)) return
    if (skipIfMissingIds(t, setup, ["content01","newsletter01"])) return
  ...
  test('direct-list-newsletter_metric', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-newsletter_metric', setup.live)) return
    if (skipIfMissingIds(t, setup, ["newsletter_metric01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_NEWSLETTER_METRIC_ENTID']
