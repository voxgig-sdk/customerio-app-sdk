...
describe('ActionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.live) {
      return runLiveEntity(setup, {"active":true,"alias":{"field":{}},"fields":[],"name":"action","op":{},"relations":{"ancestors":[["broadcast"],["campaign"]]},"key$":"action","name__orig":"action","Name":"Action","name_":"action","name-":"action","NAME":"ACTION","index$":0}, {"active":true,"entity":"action","key$":"BasicActionFlow","kind":"basic","name":"BasicActionFlow","param":{},"step":[]}, 'Action')
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['action01','action02','action03','broadcast01','broadcast02','broadcast03','campaign01','campaign02','campaign03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  idmap = env['CUSTOMERIO_APP_TEST_ACTION_ENTID']

  const live = 'TRUE' === env.CUSTOMERIO_APP_TEST_LIVE

  const transport = createLiveTransport()
  if (live) {
    const rawIds = process.env['CUSTOMERIO_APP_TEST_ACTION_ENTID']
