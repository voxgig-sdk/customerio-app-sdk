...
describe('DesignStudioEmailDirect', async () => {
  ...
  test('direct-load-design_studio_email', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-design_studio_email', setup.live)) return
    if (skipIfMissingIds(t, setup, ["client01","email01","inbox_preview01"])) return
  ...
  test('direct-list-design_studio_email', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-design_studio_email', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_DESIGN_STUDIO_EMAIL_ENTID']
