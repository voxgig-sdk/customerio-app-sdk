...
describe('TriggerEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.live) {
      return runLiveEntity(setup, {"active":true,"alias":{"field":{}},"fields":[],"name":"trigger","op":{},"relations":{"ancestors":[["campaign"]]},"key$":"trigger","name__orig":"trigger","Name":"Trigger","name_":"trigger","name-":"trigger","NAME":"TRIGGER","index$":49}, {"active":true,"entity":"trigger","key$":"BasicTriggerFlow","kind":"basic","name":"BasicTriggerFlow","param":{},"step":[]}, 'Trigger')
    }
    ...
    let trigger_ref01_data = Object.values(setup.data.existing.trigger)[0] as any
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['trigger01','trigger02','trigger03','campaign01','campaign02','campaign03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  idmap = env['CUSTOMERIO_APP_TEST_TRIGGER_ENTID']

  const live = 'TRUE' === env.CUSTOMERIO_APP_TEST_LIVE

  const transport = createLiveTransport()
  if (live) {
    const rawIds = process.env['CUSTOMERIO_APP_TEST_TRIGGER_ENTID']
