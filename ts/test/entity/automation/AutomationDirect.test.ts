...
describe('AutomationDirect', async () => {
  ...
  test('direct-load-automation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-automation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["action01","campaign01"])) return
  ...
  test('direct-list-automation', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-automation', setup.live)) return
    if (skipIfMissingIds(t, setup, ["campaign01"])) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CUSTOMERIO_APP_TEST_AUTOMATION_ENTID']
