# CustomerioApp TypeScript SDK



The TypeScript SDK for the CustomerioApp API — a type-safe, entity-oriented client with full async/await support.
...
| `customer_identifiers` | `Record<string, any>` | Identifiers for the person in a response—`id`, `cio_id`, and `email`. |
| `data` | `any` |  |
...
| `language_variants` | `Record<string, any>` | Metrics for each language variant of a multi-language message, keyed by the variant's action ID. |
...
| `metric` | `Record<string, any>` | Contains metrics for the link. |
| `metrics` | `Record<string, any>` | Metrics for an individual instance of a message; each item in the object represents the timestamp when a message achieved a particular metric. |
...
| `series` | `Record<string, any>` | Metrics grouped by the requested resolution. |
...
| `tracked_responses` | `Record<string, any>` | Tracked in-app survey responses, keyed by response option name. |
...
| `content` | `Record<string, any>` | The content of your email. |
...
| `emails` | `any[]` |  |
| `envelope` | `Record<string, any>` | The envelope of your email, like from and to addresses. |
...
| `node` | `Record<string, any>` | The content and settings stored in the version. |
...
| `sample_data` | `Record<string, any>` | Liquid variables to render with, as a JSON object—any shape is accepted, from flat variables like `{"first_name": "Janine"}` to nested ones like `{"customer": {"first_name": "Janine"}}`. |
...
| `transformers` | `Record<string, any>` | Automate repetitive actions like removing white space and inlining CSS with [transformers](/journeys/design-studio/emails/code-editor/overview/#transformers). |
...
| Field | Type | Description |
| --- | --- | --- |
| `attributes` | `Record<string, any>` | Attributes assigned to this object. |
...
| `identifiers` | `Record<string, any>` | Identifies an object. |
...
| `timestamps` | `Record<string, any>` | The epoch timestamps when corresponding attributes were set on the object. |
...
| `tags` | `any[] | null` | The tags assigned to the segment, if any. |
...
| Field | Type | Description |
| --- | --- | --- |
| `attachments` | `Record<string, any>` | A dictionary of attachments where the filename is the key and the value is the base64-encoded contents. |
...
| `custom_data` | `Record<string, any>` | Optional key/value pairs you want to attach to the push payload. |
| `custom_device` | `any` | A device to perform an upsert operation at the time of send. |
| `custom_payload` | `Record<string, any>` | Optional key/value pairs you want to attach to the push payload. |
...
| `message_data` | `Record<string, any>` | An object containing the key-value pairs referenced using liquid in your message. |
...
customerio-app/
├── src/
│   ├── CustomerioAppSDK.ts        # Main SDK class
