...
class CustomerioAppMakeError
{
    public static function call(?CustomerioAppContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CustomerioAppContext([], null);
        }
        $op = $ctx->op ?? new CustomerioAppOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CustomerioAppResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CustomerioAppError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CustomerioAppError('', $msg, $ctx);
        ...
        if ($err instanceof CustomerioAppError) {
