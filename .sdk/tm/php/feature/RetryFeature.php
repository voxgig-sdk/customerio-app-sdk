...
// CustomerioApp SDK retry feature
...
class CustomerioAppRetryFeature extends CustomerioAppBaseFeature
{
    ...
    public function init(CustomerioAppContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CustomerioAppContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CustomerioAppContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CustomerioAppContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
