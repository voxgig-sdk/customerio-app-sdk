...
from projectname_sdk import CustomerioAppSDK
from projectname_sdk.core.error import CustomerioAppError
from projectname_sdk.core.result import CustomerioAppResult
from projectname_sdk.core.response import CustomerioAppResponse
from projectname_sdk.core.spec import CustomerioAppSpec
from projectname_sdk.feature.base_feature import CustomerioAppBaseFeature


def _client():
    return CustomerioAppSDK.test(None, None)
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = CustomerioAppSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResponse:
    ...
    def test_guards_missing_spec_response_result(self):
        ...
        ctx.result = CustomerioAppResult({})
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = CustomerioAppResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = CustomerioAppResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = CustomerioAppResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = CustomerioAppResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = CustomerioAppResult({"ok": False, "resdata": "fallback"})
...
class TestFeatureOrdering:
    ...
    def _named_feature(self, name):
        feature = CustomerioAppBaseFeature()
        feature.name = name
        return feature
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else CustomerioAppSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = CustomerioAppResponse({"status": 200})
        ctx.result = CustomerioAppResult({})
